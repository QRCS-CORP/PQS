...
static void client_connection_dispose(pqs_connection_state* cns)
{
...
static void client_connection_login(pqs_connection_state* cns)
{
	...
	uint8_t smsg[PQS_HEADER_SIZE + PQS_SIMPLEX_HASH_SIZE + PQS_SIMPLEX_MACTAG_SIZE] = { 0 };
	size_t mlen;
	size_t slen;
...
static size_t client_format_message(const char* message, size_t msglen)
{
	char tmps[1024] = { 0 };
	int64_t len;
	size_t pos;
	...
	while (true)
	{
		...
		len = qsc_stringutils_find_string(message + pos, "\n");

		if (len > 0)
		{
			if ((size_t)len < sizeof(tmps))
			{
				qsc_memutils_copy(tmps, message + pos, (size_t)len);

				if (first == false)
				{
					printf_s("%s %s \n", m_client_connection_state.prompt, tmps);
				}
				else
				{
					printf_s("%s \n", tmps);
				...
				qsc_memutils_clear(tmps, sizeof(tmps));
			}
		...
		{
			/* null char seperated strings in message */
			if (pos < msglen)
			{
				++pos;
			}
...
static void client_receive_callback(pqs_connection_state* cns, const uint8_t* message, size_t msglen)
{
	...
	{
		client_format_message((char*)message, msglen);

		if ((char)message[msglen - 1] != '\n')
		{
			client_print_message("");
		}
		else
...
static void client_send_loop(pqs_connection_state* cns)
{
	...
	if (bres == true)
	{
		char sin[PQS_CLIENT_INPUT_MAX + 1] = { 0 };
		size_t mlen;
		...
		while (true)
		{
			...
			{
				pqs_network_packet spkt = { 0 };
				uint8_t msg[PQS_CONNECTION_MTU] = { 0 };

				/* cache the command */
				m_client_connection_state.command = client_command_from_string(sin);

				if (m_client_connection_state.command == client_command_execute)
				{
					/* convert the packet to bytes */
				...
				else if (m_client_connection_state.command == client_command_quit)
				{
					pqs_connection_close(cns, pqs_error_none, true);
					break;
				}
				else if (m_client_connection_state.command == client_command_cprint)
				{
					client_print_pubkey();
					client_print_message("");
				}
...
int main(void)
{
	...
	if (res == true)
	{
		perr = pqs_client_simplex_connect_ipv4(&m_client_connection_state.pubkey, &addv4t, PQS_SERVER_PORT, &client_send_loop, &client_receive_callback);
